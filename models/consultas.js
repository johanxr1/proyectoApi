...
export default class Model {
  ...
  getAllDetalled = async (table, callback) => {
    let sql = `SELECT alimentos.*, f.nombre AS fuente_nombre, g.nombre AS grupo_nombre FROM alimentos INNER JOIN fuentes as f on f.id = alimentos.fuente INNER JOIN grupos as g ON g.id = alimentos.grupo`;
    pool.query(sql, callback);
  };
}
